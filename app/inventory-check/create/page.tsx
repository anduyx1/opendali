...
export default function CreateInventoryCheckPage() {
  ...
  const addProduct = async (product: ProductSearch) => {
    ...
    try {
      await addProductToCheck(Number(currentSessionId), Number(product.id))
  ...
  const updateProductStock = async (id: string, actualStock: number) => {
    ...
    try {
      ...
      const result = await updateProductActualStock(Number(currentSessionId), Number(id), actualStock)
